...
function fetchItems(searchTerm) {
    fetch(`https://medical-api-dev-amkp.2.ie-1.fl0.io/search?search=${searchTerm}`)
        .then(response => {
            if (!response.ok) {
...
async function fetchItemsWithFilter(filterBy) {
    try {
        ...
        const response = await fetch(`https://medical-api-dev-amkp.2.ie-1.fl0.io/filterBy?filter=${filterBy}`);
...
async function fetchAndDisplayItems() {
    try {
        ...
        const response = await fetch('https://medical-api-dev-amkp.2.ie-1.fl0.io/items');
        const items = await response.json();
        console.log(items)
