...
function fetchItems(searchTerm) {
    fetch(`https://medical-api-dev-tqcc.2.ie-1.fl0.io/search?search=${searchTerm}`)
        .then(response => {
            if (!response.ok) {
...
async function fetchItemsWithFilter(filterBy) {
    try {
        ...
        const response = await fetch(`https://medical-api-dev-tqcc.2.ie-1.fl0.io/filterBy?filter=${filterBy}`);
...
async function fetchAndDisplayItems() {
    try {
        ...
        const response = await fetch('https://medical-api-dev-tqcc.2.ie-1.fl0.io/items');
        const items = await response.json();
        console.log(items)
    ...
    items.forEach(item => {
        ...
        itemDiv.innerHTML = `
            <div class="info">
                <div id="itemName">
                ${item.nombre}
                ${item.estupefacientes ? '<span class="itemTag estu">Estupefaciente</span>' : ''}
                ${item.termolabil ? '<span class="itemTag termo">termolabil</span>' : ''}
                ${item['alto riesgo'] ? '<span class="itemTag riesgo">alto riesgo</span>' : ''}
                ${item.mezclas ? '<span class="itemTag mezcla">mezcla</span>' : ''}
                </div>
